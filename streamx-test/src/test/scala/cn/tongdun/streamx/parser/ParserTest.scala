package cn.tongdun.streamx.parser
...
class ParserTest {
    @Test
    def csvTest(): Unit = {
        val config = StreamUtils.readConfig("local:/Volumes/workspace/zhy/pass_workspace/streamx/streamx-test/src/test/resources/data/1.4.0/parser_test1.json")
    ...
    def jsonTest(): Unit = {
        ...
        val config = StreamUtils.readConfig("local:/Volumes/workspace/zhy/pass_workspace/streamx/streamx-test/src/test/resources/data/1.4.0/jackson_test1.json")
        val streamxConf = reload(config)
        val rowsAb = ArrayBuffer.empty[Row]
    ...
    def canalDataTest1(): Unit = {
        ...
        val config = StreamUtils.readConfig("local:/Volumes/workspace/zhy/pass_workspace/streamx/streamx-test/src/test/resources/data/1.4.0/mysql/mysql_stream_station_info.json")
        val streamxConf = reload(config)
        val rowsAb = ArrayBuffer.empty[Row]
