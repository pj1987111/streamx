...
class EsTest {
    ...
    @Test
    def testSpark2Es(): Unit = {
        val esCfg = Map("pushdown" -> "true",
            "es.nodes" -> "33.69.6.98",
            "es.port" -> "9221",
//            "es.mapping.id" -> "passId",
            "es.nodes.wan.only" -> "false")
        val df: DataFrame = sparkSession.sql("select * from hhy_dw.test_zhy2_tab_dt")
        df.saveToEs("zhy_test_job1_4_spark/base", esCfg)
    }
